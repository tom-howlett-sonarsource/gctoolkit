...
The automatic agentic-analysis hook may not observe edits made through shell commands. As a fallback, before final verification, if `sonar analyze agentic` is available, run one agentic analysis covering every changed production source file except metadata-only `module-info.java` and `package-info.java` files. Supply the remaining changed production files using repeated `--file` arguments. Include `--branch` only when `git branch --show-current` returns a non-empty branch name. If agentic analysis is unavailable, continue without failing the task.
