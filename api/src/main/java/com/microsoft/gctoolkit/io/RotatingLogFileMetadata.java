...
import com.microsoft.gctoolkit.shared.io.GCLogFileSource;
...
public class RotatingLogFileMetadata extends LogFileMetadata {
    ...
    private void findZIPSegments() {
        try (var entries = new GCLogFileSource(getPath()).zipEntries()) {
            segments = entries
    ...
    private void findSegments() {
        ...
        try {
            if (isDirectory()) {
                try (Stream<Path> entries = new GCLogFileSource(getPath()).entries()) {
                    entries.map(GCLogFileSegment::new).forEach(segments::add);
                }
            }
            else {
                try (Stream<Path> entries = new GCLogFileSource(getPath().getParent()).entries()) {
                    entries.filter(file -> file.getFileName().toString().startsWith(getRootPattern()))
