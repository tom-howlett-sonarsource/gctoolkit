...
public class RotatingLogFileMetadata extends LogFileMetadata {
    ...
    private void findZIPSegments() {
        try {
            segments = LogSourceDiscovery.zipEntryNames(getPath()).stream()
                    .<LogFileSegment>map(name -> new GCLogFileZipSegment(getPath(), name))
                    .collect(toList());
    ...
    private void findSegments() {
        ...
        try {
            if (isDirectory()) {
                LogSourceDiscovery.pathsIn(getPath()).stream()
                        .map(GCLogFileSegment::new).forEach(segments::add);
            }
            else {
                LogSourceDiscovery.pathsIn(getPath().getParent()).stream()
