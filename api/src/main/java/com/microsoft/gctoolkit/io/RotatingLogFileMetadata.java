...
public class RotatingLogFileMetadata extends LogFileMetadata {
    ...
    private void findSegments() {
        ...
        try {
            if (isDirectory()) {
                GCLogSource.discover(getPath(), path -> true)
                        .stream().map(GCLogFileSegment::new).forEach(segments::add);
            }
            else {
                String rootPattern = getRootPattern();
                GCLogSource.discover(getPath().getParent(),
                                file -> file.getFileName().toString().startsWith(rootPattern))
                        .stream().map(GCLogFileSegment::new).forEach(segments::add);
            }
        } catch (IOException ioe) {
