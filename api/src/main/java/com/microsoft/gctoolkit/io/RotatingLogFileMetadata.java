...
public class RotatingLogFileMetadata extends LogFileMetadata {
    ...
    private void findZIPSegments() {
        try {
            segments = LogSourceFiles.zipEntryNames(getPath()).stream()
    ...
    private void findSegments() {
        ...
        try {
            if (isDirectory()) {
                LogSourceFiles.list(getPath()).stream().map(GCLogFileSegment::new).forEach(segments::add);
            }
            else {
                LogSourceFiles.list(getPath().getParent()).stream()
