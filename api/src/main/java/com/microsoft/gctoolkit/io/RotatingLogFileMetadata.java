...
public class RotatingLogFileMetadata extends LogFileMetadata {
    ...
    private void findZIPSegments() {
        try {
            segments = GCLogSource.zipEntries(getPath()).stream()
    ...
    private void findSegments() {
        segments = new ArrayList<>();
        try (Stream<Path> sources = GCLogSource.list(isDirectory() ? getPath() : getPath().getParent())) {
            if (isDirectory()) {
                sources.map(GCLogFileSegment::new).forEach(segments::add);
            }
            else {
                sources.filter(file -> file.getFileName().toString().startsWith(getRootPattern()))
