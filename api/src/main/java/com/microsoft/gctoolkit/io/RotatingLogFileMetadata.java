...
public class RotatingLogFileMetadata extends LogFileMetadata {
    ...
    private void findZIPSegments() {
        try {
            segments = LogSources.discover(getPath()).stream()
    ...
    private void findSegments() {
        ...
        try {
            if (isDirectory()) {
                LogSources.discover(getPath()).stream()
                        .map(LogSource::getPath)
                        .map(GCLogFileSegment::new)
                        .forEach(segments::add);
            }
            else {
                LogSources.discover(getPath().getParent()).stream()
                        .filter(source -> source.getName().startsWith(getRootPattern()))
                        .map(LogSource::getPath)
                        .map(GCLogFileSegment::new)
                        .forEach(segments::add);
            }
