...
import com.microsoft.gctoolkit.logsource.LogFileDiscovery;
...
public class RotatingLogFileMetadata extends LogFileMetadata {
    ...
    private void findZIPSegments() {
        segments = new ArrayList<>();
        try {
            LogFileDiscovery.zipEntryNames(getPath())
                    .stream()
    ...
    private void findSegments() {
        segments = new ArrayList<>();
        try {
            List<Path> logFiles = isDirectory()
                    ? LogFileDiscovery.directoryContents(getPath())
                    : LogFileDiscovery.siblingsWithPrefix(getPath(), getRootPattern());
            logFiles.stream().map(GCLogFileSegment::new).forEach(segments::add);
        } catch (IOException ioe) {
