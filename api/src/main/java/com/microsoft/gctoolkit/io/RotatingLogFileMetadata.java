...
import com.microsoft.gctoolkit.io.source.LogSourceDiscovery;
...
public class RotatingLogFileMetadata extends LogFileMetadata {
    ...
    private void findSegments() {
        ...
        try {
            if (isDirectory()) {
                LogSourceDiscovery.listDirectory(getPath())
                        .stream()
                        .map(GCLogFileSegment::new)
                        .forEach(segments::add);
            }
            else {
                LogSourceDiscovery.listSiblingsStartingWith(getPath(), getRootPattern())
                        .stream()
                        .map(GCLogFileSegment::new)
                        .forEach(segments::add);
            }
