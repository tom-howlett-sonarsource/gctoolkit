...
public class RotatingLogFileMetadata extends LogFileMetadata {
    ...
    private void findZIPSegments() {
        try {
            segments = LogSource.discover(getPath()).stream()
                    .map(LogSource::entryName)
    ...
    private void findSegments() {
        ...
        try {
            if (isDirectory()) {
                LogSource.discoverPaths(getPath()).stream().map(GCLogFileSegment::new).forEach(segments::add);
            }
            else {
                LogSource.discoverPaths(getPath().getParent()).stream()
