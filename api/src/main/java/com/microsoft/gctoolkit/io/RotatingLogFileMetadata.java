...
public class RotatingLogFileMetadata extends LogFileMetadata {
    ...
    private void findZIPSegments() {
        segments = GCLogSources.zipEntryNames(getPath()).stream()
                .map(name -> (LogFileSegment) new GCLogFileZipSegment(getPath(), name))
                .collect(toList());
        orderSegments();
    }
    ...
    private void findSegments() {
        ...
        try {
            if (isDirectory()) {
                GCLogSources.listFiles(getPath()).stream().map(GCLogFileSegment::new).forEach(segments::add);
            }
            else {
                GCLogSources.listFilesStartingWith(getPath().getParent(), getRootPattern())
                        .stream()
