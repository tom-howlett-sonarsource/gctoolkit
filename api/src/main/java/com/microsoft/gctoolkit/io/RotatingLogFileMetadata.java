...
public class RotatingLogFileMetadata extends LogFileMetadata {
    ...
    private void findZIPSegments() {
        try {
            segments = LogFileSources.discover(getPath()).stream()
                    .map(LogFileSource::name)
    ...
    private void findSegments() {
        ...
        try {
            if (isDirectory()) {
                LogFileSources.files(getPath()).stream().map(GCLogFileSegment::new).forEach(segments::add);
            }
            else {
                LogFileSources.files(getPath().getParent()).stream()
