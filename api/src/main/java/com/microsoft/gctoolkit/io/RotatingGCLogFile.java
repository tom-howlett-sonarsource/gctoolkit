...
public class RotatingGCLogFile extends GCLogFile {
    ...
    public Stream<String> stream() throws IOException {
        ...
        if (metadata.isDirectory()
                ...
                || metadata.isZip()) {
            Stream<String> segmentLines = metadata.logFiles()
                    .flatMap(LogFileSegment::stream)
                    .filter(Objects::nonNull)
                    .map(String::trim)
                    .filter(s -> s.length() > 0);
            return Stream.concat(segmentLines, Stream.of(endOfData()));
    ...
    private Stream<String> streamZipFile() throws IOException {
        ...
        try {
            ...
            return ResourceClosingStreams.lines(reader, zipFile);
        } catch (IOException | RuntimeException | Error failure) {
            ResourceClosingStreams.closeAfterFailure(failure, zipFile);
            throw failure;
        }
    }
