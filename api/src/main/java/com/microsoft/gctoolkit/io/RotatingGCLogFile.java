...
public class RotatingGCLogFile extends GCLogFile {
    ...
    public Stream<String> stream() throws IOException {
        ...
        if (metadata.isDirectory() || metadata.isPlainText() || metadata.isZip()) {
            Stream<String> segments = concatenate(metadata.logFiles());
    ...
    private Stream<String> concatenate(Stream<LogFileSegment> segments) {
        return segments
    ...
    private Stream<String> streamZipFile() throws IOException {
        return concatenate(getMetaData().logFiles());
    }
