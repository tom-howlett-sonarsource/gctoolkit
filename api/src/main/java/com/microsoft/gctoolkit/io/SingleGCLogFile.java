...
import com.microsoft.gctoolkit.logsource.GCLogSourceStreams;
...
public class SingleGCLogFile extends GCLogFile {
    ...
    private Stream<String> stream(LogFileMetadata metadata) throws IOException {
        ...
        if (metadata.isPlainText()) {
            stream = GCLogSourceStreams.streamPlainText(metadata.getPath());
        } else if (metadata.isZip()) {
            stream = GCLogSourceStreams.streamZipFile(metadata.getPath());
        } else if (metadata.isGZip()) {
            stream = GCLogSourceStreams.streamGZipFile(metadata.getPath());
        }
        if ( stream == null)
