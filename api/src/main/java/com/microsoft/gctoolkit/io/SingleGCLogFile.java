...
public class SingleGCLogFile extends GCLogFile {
    ...
    private static Stream<String> streamZipFile(Path path) throws IOException {
        ...
        try {
            ...
            return LogFileStreams.lines(zipStream);
        } catch (IOException ioe) {
            LogFileStreams.closeQuietly(zipStream);
            throw ioe;
        }
    }
    ...
    private static Stream<String> streamGZipFile(Path path) throws IOException {
        ...
        try {
            return LogFileStreams.lines(new GZIPInputStream(compressed));
        } catch (IOException ioe) {
            LogFileStreams.closeQuietly(compressed);
            throw ioe;
        }
    }
