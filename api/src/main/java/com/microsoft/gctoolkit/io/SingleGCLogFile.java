...
public class SingleGCLogFile extends GCLogFile {
    ...
    private Stream<String> stream(LogFileMetadata metadata) throws IOException {
        ...
        if (metadata.isPlainText()) {
            stream = LogSourceIO.stream(LogSourceFormat.PLAINTEXT, metadata.getPath());
        } else if (metadata.isZip()) {
            stream = LogSourceIO.stream(LogSourceFormat.ZIP, metadata.getPath());
        } else if (metadata.isGZip()) {
            stream = LogSourceIO.stream(LogSourceFormat.GZIP, metadata.getPath());
        }
        if ( stream == null)
