...
public class SingleGCLogFile extends GCLogFile {
    ...
    private static Stream<String> streamGZipFile(Path path) throws IOException {
        ...
        try {
            return ResourceStreams.lines(new BufferedReader(new InputStreamReader(
                    new BufferedInputStream(new GZIPInputStream(inputStream)))));
        } catch (IOException | RuntimeException exception) {
            ResourceStreams.closeAfterFailure(exception, inputStream);
            throw exception;
        }
    }
