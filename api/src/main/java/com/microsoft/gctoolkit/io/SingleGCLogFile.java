...
import com.microsoft.gctoolkit.logfile.GCLogSource;
...
public class SingleGCLogFile extends GCLogFile {
    ...
    private Stream<String> stream(LogFileMetadata metadata) throws IOException {
        ...
        if (metadata.isPlainText()) {
            stream = GCLogSource.streamPlainText(metadata.getPath());
        } else if (metadata.isZip()) {
            stream = GCLogSource.streamZipFile(metadata.getPath());
        } else if (metadata.isGZip()) {
            stream = GCLogSource.streamGZipFile(metadata.getPath());
        }
        if ( stream == null)
