...
public class SingleGCLogFile extends GCLogFile {
    ...
    private static Stream<String> streamZipFile(Path path) throws IOException {
        ...
        try {
            ...
            return LineStreams.lines(zipStream);
        } catch (IOException | RuntimeException e) {
            LineStreams.closeSuppressing(zipStream, e);
            throw e;
        }
    }
    ...
    private static Stream<String> streamGZipFile(Path path) throws IOException {
        ...
        try {
            return LineStreams.lines(new GZIPInputStream(fileStream));
        } catch (IOException | RuntimeException e) {
            LineStreams.closeSuppressing(fileStream, e);
            throw e;
        }
    }
