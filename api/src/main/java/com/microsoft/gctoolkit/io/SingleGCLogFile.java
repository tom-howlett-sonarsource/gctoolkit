...
public class SingleGCLogFile extends GCLogFile {
    ...
    private Stream<String> stream(LogFileMetadata metadata) throws IOException {
        LogSourceFormat format = formatOf(metadata);
        if (format == LogSourceFormat.UNKNOWN)
            throw new IOException("Unable to read " + path.toString());
        Stream<String> stream = LogSources.openLines(metadata.getPath(), format);
    ...
    private static LogSourceFormat formatOf(LogFileMetadata metadata) {
        if (metadata.isPlainText()) return LogSourceFormat.PLAIN_TEXT;
        if (metadata.isZip()) return LogSourceFormat.ZIP;
        if (metadata.isGZip()) return LogSourceFormat.GZIP;
