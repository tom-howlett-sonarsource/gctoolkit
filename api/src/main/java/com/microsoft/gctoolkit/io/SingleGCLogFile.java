...
import com.microsoft.gctoolkit.io.core.GCLogSource;
...
public class SingleGCLogFile extends GCLogFile {
    ...
    private Stream<String> stream(LogFileMetadata metadata) throws IOException {
        ...
        if (metadata.isPlainText()) {
            stream = GCLogSource.openPlainTextStream(metadata.getPath());
        } else if (metadata.isZip()) {
            stream = GCLogSource.openZipStream(metadata.getPath());
        } else if (metadata.isGZip()) {
            stream = GCLogSource.openGZipStream(metadata.getPath());
        }
        if ( stream == null)
