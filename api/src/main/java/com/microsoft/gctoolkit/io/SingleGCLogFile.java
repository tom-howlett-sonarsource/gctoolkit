...
import com.microsoft.gctoolkit.io.support.LogFileStreams;
...
public class SingleGCLogFile extends GCLogFile {
    ...
    private Stream<String> stream(LogFileMetadata metadata) throws IOException {
        ...
        if (metadata.isPlainText()) {
            stream = LogFileStreams.openPlainText(metadata.getPath());
        } else if (metadata.isZip()) {
            stream = LogFileStreams.openZip(metadata.getPath());
        } else if (metadata.isGZip()) {
            stream = LogFileStreams.openGZip(metadata.getPath());
        }
        if ( stream == null)
