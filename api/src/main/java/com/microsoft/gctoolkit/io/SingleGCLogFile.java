...
import com.microsoft.gctoolkit.io.source.LogStreamSource;
...
public class SingleGCLogFile extends GCLogFile {
    ...
    private Stream<String> stream(LogFileMetadata metadata) throws IOException {
        Stream<String> stream = LogStreamSource.stream(metadata.getPath(),
                toLogStreamFormat(metadata));
    ...
    private static LogStreamSource.FileFormat toLogStreamFormat(LogFileMetadata metadata) {
        if (metadata.isZip()) {
            return LogStreamSource.FileFormat.ZIP;
        } else if (metadata.isGZip()) {
            return LogStreamSource.FileFormat.GZIP;
        }
        return LogStreamSource.FileFormat.PLAINTEXT;
    }
