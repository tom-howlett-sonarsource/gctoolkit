...
public class SingleGCLogFile extends GCLogFile {
    ...
    private Stream<String> stream(LogFileMetadata metadata) throws IOException {
        ...
        if (metadata.isPlainText()) {
            stream = LogSources.open(metadata.getPath(), SourceFormat.PLAINTEXT);
        } else if (metadata.isZip()) {
            stream = LogSources.open(metadata.getPath(), SourceFormat.ZIP);
        } else if (metadata.isGZip()) {
            stream = LogSources.open(metadata.getPath(), SourceFormat.GZIP);
        }
        if ( stream == null)
