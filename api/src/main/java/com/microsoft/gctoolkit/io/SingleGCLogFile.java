...
public class SingleGCLogFile extends GCLogFile {
    ...
    private static Stream<String> streamZipFile(Path path) throws IOException {
        ...
        try {
            ...
            return StreamResources.closeWith(reader.lines(), reader, zipStream);
        } catch (IOException | RuntimeException failure) {
            StreamResources.closeAfterFailure(failure, zipStream);
            throw failure;
        }
    }
    ...
    private static Stream<String> streamGZipFile(Path path) throws IOException {
        ...
        try {
            ...
            return StreamResources.closeWith(reader.lines(), reader, gzipStream);
        } catch (IOException | RuntimeException failure) {
            StreamResources.closeAfterFailure(failure, inputStream);
            throw failure;
        }
    }
