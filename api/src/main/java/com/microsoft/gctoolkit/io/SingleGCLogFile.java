...
public class SingleGCLogFile extends GCLogFile {
    ...
    private Stream<String> stream(LogFileMetadata metadata) throws IOException {
        ...
        if (metadata.isPlainText()) {
            stream = streamSource(metadata.getPath());
    ...
    private static Stream<String> streamSource(Path path) throws IOException {
        return new BufferedReader(new InputStreamReader(GCLogSource.open(path))).lines();
    }
