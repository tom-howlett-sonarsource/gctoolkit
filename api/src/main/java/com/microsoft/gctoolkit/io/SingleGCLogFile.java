...
public class SingleGCLogFile extends GCLogFile {
    ...
    private static Stream<String> streamZipFile(Path path) throws IOException {
        ...
        try {
            ...
            return closeableLines(zipStream);
        } catch (IOException | RuntimeException exception) {
            closeOnFailure(zipStream, exception);
            throw exception;
        }
    }
    ...
    private static Stream<String> streamGZipFile(Path path) throws IOException {
        ...
        try {
            return closeableLines(new GZIPInputStream(fileStream));
        } catch (IOException | RuntimeException exception) {
            closeOnFailure(fileStream, exception);
            throw exception;
        }
    }

    private static Stream<String> closeableLines(InputStream inputStream) {
        BufferedReader reader = new BufferedReader(new InputStreamReader(
                new BufferedInputStream(inputStream), Charset.defaultCharset()));
    ...
    private static void closeOnFailure(InputStream inputStream, Throwable failure) {
        try {
            inputStream.close();
