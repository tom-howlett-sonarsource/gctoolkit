...
public class SingleGCLogFile extends GCLogFile {
    ...
    private Stream<String> stream(LogFileMetadata metadata) throws IOException {
        ...
        if (metadata.isPlainText()) {
            stream = LogSourceIO.stream(metadata.getPath(), LogSourceFormat.PLAINTEXT);
        } else if (metadata.isZip()) {
            stream = LogSourceIO.stream(metadata.getPath(), LogSourceFormat.ZIP);
        } else if (metadata.isGZip()) {
            stream = LogSourceIO.stream(metadata.getPath(), LogSourceFormat.GZIP);
        }
        if ( stream == null)
