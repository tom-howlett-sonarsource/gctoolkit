...
import com.microsoft.gctoolkit.gclogsource.GCLogSourceStreams;
...
public class SingleGCLogFile extends GCLogFile {
    ...
    private Stream<String> stream(LogFileMetadata metadata) throws IOException {
        Stream<String> stream = GCLogSourceStreams.lines(metadata.getPath(), GCLogSourceFormat.from(metadata.getPath()));
        return Stream.concat(GCLogSourceStreams.normalized(stream), Stream.of(endOfData()));
    }
