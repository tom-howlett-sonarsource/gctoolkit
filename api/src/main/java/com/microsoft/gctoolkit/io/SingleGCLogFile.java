...
import com.microsoft.gctoolkit.logsource.GCLogStreams;
...
public class SingleGCLogFile extends GCLogFile {
    ...
    private Stream<String> stream(LogFileMetadata metadata) throws IOException {
        ...
        if (metadata.isPlainText()) {
            stream = GCLogStreams.openPlain(metadata.getPath());
        } else if (metadata.isZip()) {
            stream = GCLogStreams.openZip(metadata.getPath());
        } else if (metadata.isGZip()) {
            stream = GCLogStreams.openGZip(metadata.getPath());
        }
        if ( stream == null)
