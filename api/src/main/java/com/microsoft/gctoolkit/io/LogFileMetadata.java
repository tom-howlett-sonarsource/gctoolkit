...
import com.microsoft.gctoolkit.log.source.LogFileFormat;
import com.microsoft.gctoolkit.log.source.LogFileSources;
...
public abstract class LogFileMetadata {
    ...
    boolean magic(int field1, int field2) {
        return LogFileSources.matchesMagic(path, field1, field2);
    }

    public abstract Stream<LogFileSegment> logFiles();

    private static FileFormat classify(Path path) {
        LogFileFormat detected = LogFileSources.detectFormat(path);
        switch (detected) {
            case DIRECTORY:
