...
public abstract class LogFileMetadata {
    ...
    private void discoverFormat() throws IOException {
        switch (LogSource.discover(path)) {
            ...
            case PLAIN: fileFormat = FileFormat.PLAINTEXT; break;
            default: fileFormat = FileFormat.UNKNOWN;
        }
    }

    /** Return the stored size of this source in bytes. */
    public long getByteSize() throws IOException {
        return LogSource.byteSize(path);
    }
