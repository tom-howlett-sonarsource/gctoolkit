...
import com.microsoft.gctoolkit.source.GCLogSources;
import com.microsoft.gctoolkit.source.LogSourceFormat;
...
public abstract class LogFileMetadata {
    ...
    public LogFileMetadata(Path path) throws IOException {
        this.path = path;
        this.fileFormat = FileFormat.from(GCLogSources.detectFormat(path));
    }
    ...
    enum FileFormat {
        ...
        static FileFormat from(LogSourceFormat format) {
            switch (format) {
                case ZIP:
                    return ZIP;
                case GZIP:
