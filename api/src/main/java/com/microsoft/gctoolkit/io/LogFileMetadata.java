...
public abstract class LogFileMetadata {
    ...
    private void magic() {
        try {
            GCLogSource.Format format = GCLogSource.discover(getPath());
            if (format == GCLogSource.Format.DIRECTORY)
                fileFormat = FileFormat.DIRECTORY;
            else if (format == GCLogSource.Format.GZIP)
                fileFormat = FileFormat.GZIP;
            else if (format == GCLogSource.Format.ZIP)
