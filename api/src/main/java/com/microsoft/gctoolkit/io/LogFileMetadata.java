...
public abstract class LogFileMetadata {
    ...
    public LogFileMetadata(Path path) throws IOException {
        this.path = path;
        LogSourceFormat detectedFormat;
        try {
            detectedFormat = LogSourceIO.detectFormat(path);
        } catch (IOException exception) {
            LOG.log(Level.WARNING, "Unable to detect log file format.", exception);
            detectedFormat = LogSourceFormat.PLAINTEXT;
        }
