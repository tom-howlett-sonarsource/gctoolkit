...
import com.microsoft.gctoolkit.shared.io.LogSourceIO;
...
public abstract class LogFileMetadata {
    ...
    public LogFileMetadata(Path path) throws IOException {
        this.path = path;
        fileFormat = toFileFormat(LogSourceIO.format(path));
    }
    ...
    private static FileFormat toFileFormat(LogSourceIO.Format format) {
        switch (format) {
            case DIRECTORY:
