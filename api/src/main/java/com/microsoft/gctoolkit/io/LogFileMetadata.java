...
public abstract class LogFileMetadata {
    ...
    private final GCLogSourceFormat fileFormat;
    private final Path path;

    protected LogFileMetadata(Path path) throws IOException {
        this.path = path;
        this.fileFormat = GCLogSources.detect(path);
    }
    ...
    /**
     * The detected source format for the underlying path.
     * @return The {@link GCLogSourceFormat} discovered when this metadata was created.
     */
    public GCLogSourceFormat getSourceFormat() {
        return fileFormat;
    }
}
