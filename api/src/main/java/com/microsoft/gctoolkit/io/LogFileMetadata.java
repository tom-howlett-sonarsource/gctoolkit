...
public abstract class LogFileMetadata {
    ...
    private void discoverFormat() throws IOException {
        LogSource.Format format;
        try {
            format = LogSource.discover(path).format();
        } catch (IOException exception) {
            // Keep metadata construction compatible with the previous behavior:
            // failures are surfaced when the source is actually opened.
