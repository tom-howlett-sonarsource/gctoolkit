...
public class GCLogFileZipSegment implements LogFileSegment {
    ...
    private void ageOfJVMAtLogStart() {
        if (startTime == null) {
            try (Stream<String> lines = stream()) {
                startTime = lines.filter(s -> !s.contains(" file created "))
    ...
    public <T> Collector<T, ?, List<T>> tail(int n) {
        return LogSourceIO.tailCollector(n);
    }
    ...
    public Stream<String> stream() {
        try {
            return LogSourceIO.streamZipEntry(path, segmentName);
        } catch (IOException e) {
            LOG.log(Level.WARNING, "Unable to stream zipped log segment.", e);
        }
        return Stream.empty();
    }
