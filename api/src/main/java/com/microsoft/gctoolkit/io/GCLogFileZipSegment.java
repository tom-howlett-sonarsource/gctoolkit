...
public class GCLogFileZipSegment implements LogFileSegment {
    ...
    @SuppressWarnings("java:S2095") // The returned stream owns and closes the reader and archive.
    private Stream<String> openStream() throws IOException {
    ...
    private static void close(BufferedReader reader, ZipFile file) {
        try (ZipFile archive = file; BufferedReader lines = reader) {
            // Closing the resources is the operation performed by this block.
        } catch (IOException e) {
