...
public class GCLogFileZipSegment implements LogFileSegment {
    ...
    public Stream<String> stream() {
        ...
        try {
            ...
            if (entry == null)
                throw new IOException("Unable to find " + segmentName + " in " + path);
            BufferedReader reader = new BufferedReader(new InputStreamReader(file.getInputStream(entry)));
            ZipFile archive = file;
            // BufferedReader.lines() does not close the reader, and closing the reader does not
            // close the archive. Both have to be released when the returned stream is closed,
            // which includes the case where the caller closes a partially consumed stream.
            ...
            if (file != null)
                closeQuietly(file);
            LOGGER.log(Level.WARNING, "Unable to stream " + segmentName + " from " + path, e);
        }
