...
public class GCLogFileZipSegment implements LogFileSegment {
    ...
    public Stream<String> stream() {
        try {
            return openEntryStream(new ZipFile(path.toFile()));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private Stream<String> openEntryStream(ZipFile file) throws IOException {
        try {
            ...
            return reader.lines().onClose(() -> ZipResources.closeQuietly(reader, file));
        } catch (IOException e) {
            ZipResources.closeQuietly(file);
            throw e;
        }
    }
