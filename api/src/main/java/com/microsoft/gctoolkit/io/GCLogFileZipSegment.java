...
public class GCLogFileZipSegment implements LogFileSegment {
    ...
    /**
     * Stream the file, one line at a time. The archive backing the stream is released when the
     * returned stream is closed, whether or not it has been fully consumed.
    ...
    public Stream<String> stream() {
        ...
        try {
            ...
            return LineStreams.lines(archive.getInputStream(entry), archive);
        } catch (IOException e) {
            if (archive != null)
                LineStreams.closeSuppressing(archive, e);
            LOGGER.log(Level.WARNING, "Unable to stream " + this.segmentName + " from " + path, e);
        }
        return Stream.empty();
    }
