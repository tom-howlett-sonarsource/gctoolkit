...
public class GCLogFileZipSegment implements LogFileSegment {
    ...
    public GCLogFileZipSegment(Path path, String segmentName) {
        this(path, segmentName, ZipFileOpener.DEFAULT);
    }

    GCLogFileZipSegment(Path path, String segmentName, ZipFileOpener zipFileOpener) {
        this.path = path;
    ...
    public Stream<String> stream() {
        ...
        try {
            ...
            return CloseableStreams.lines(reader, file);
        } catch (IOException e) {
            CloseableStreams.closeAfterFailure(file, e);
            LOGGER.warning(e.getMessage());
        } catch (RuntimeException exception) {
            CloseableStreams.closeAfterFailure(file, exception);
            throw exception;
        }
