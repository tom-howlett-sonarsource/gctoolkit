...
public class GCLogFileZipSegment implements LogFileSegment {
    ...
    private void ageOfJVMAtLogStart() {
        if (startTime == null) {
            // findFirst leaves the archive open unless the stream is closed.
    ...
    public Stream<String> stream() {
        ...
        try {
            ...
            // closing the reader releases the entry stream, closing file releases the archive itself.
            lines = LineStreams.lines(new BufferedReader(new InputStreamReader(file.getInputStream(entry))), file);
            return lines;
        } catch (IOException e) {
            LOGGER.log(Level.WARNING, "Unable to read " + segmentName + " in " + path, e);
            return Stream.empty();
        } finally {
            // the caller can only close the archive through a stream it was handed.
            if (lines == null)
                LineStreams.close(file);
        }
    }
