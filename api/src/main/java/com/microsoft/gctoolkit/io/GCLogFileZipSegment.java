...
public class GCLogFileZipSegment implements LogFileSegment {
    ...
    // BufferedReader.lines() does not register a close handler on the returned
    // Stream, and a ZipFile is not closed by closing the reader over one of its
    // entries, so both must be closed explicitly here.
    private static void closeQuietly(Closeable reader, ZipFile file) {
        try {
            try {
                reader.close();
            } finally {
