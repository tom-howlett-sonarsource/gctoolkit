...
public class GCLogFileZipSegment implements LogFileSegment {
     ...
     */
    @SuppressWarnings("java:S2095") // Ownership is transferred to the returned stream's close handler.
    ...
    private static void closeAfterFailure(ZipFile file, IOException failure) {
        if (file != null) {
            try {
                file.close();
