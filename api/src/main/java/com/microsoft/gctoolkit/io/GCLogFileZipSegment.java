...
public class GCLogFileZipSegment implements LogFileSegment {
    ...
    /**
     * Stream the file, one line at a time. The returned stream holds the archive open, so the
     * caller must close it, at which point the archive and every reader over it are released.
     * @return A stream of lines from the file, or an empty stream if it cannot be read.
     */
    public Stream<String> stream() {
        ZipFile archive = null;
        try {
            archive = new ZipFile(path.toFile());
            ZipEntry entry = archive.getEntry(this.segmentName);
            if (entry == null)
                throw new IOException("Unable to find " + this.segmentName + " in " + path);
            return LogFileStreams.lines(archive.getInputStream(entry), archive);
        } catch (IOException e) {
            LogFileStreams.closeQuietly(archive);
            LOGGER.log(Level.WARNING, e, () -> "Unable to stream " + this.segmentName + " from " + path);
            return Stream.empty();
        }
    }
