...
public class SafepointLogFile implements DataSource<String> {
    ...
    public Stream<String> stream() throws IOException {
        LogSourceFormat format = GCLogSources.discover(path);
        if (format == LogSourceFormat.DIRECTORY || format == LogSourceFormat.UNKNOWN) {
            throw new IOException("Unable to read " + path.toString());
        }
        return GCLogSources.open(path, format);
    }
