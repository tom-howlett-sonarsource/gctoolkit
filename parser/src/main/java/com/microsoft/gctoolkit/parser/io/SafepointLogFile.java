...
import com.microsoft.gctoolkit.sources.io.LogFileStreams;
...
public class SafepointLogFile implements DataSource<String> {
    ...
    private static final LogFileMetadata metadata = null;
    ...
    public Stream<String> stream() throws IOException {
        if (metadata.isPlainText()) {
            return LogFileStreams.openPlain(path);
        } else if (metadata.isZip()) {
            return LogFileStreams.openZip(path);
        } else if (metadata.isGZip()) {
            return LogFileStreams.openGZip(path);
        }
