...
public class SafepointLogFile implements DataSource<String> {
    ...
    public Stream<String> stream() throws IOException {
        LogSource.Format format = LogSource.discover(path);
        if (format == LogSource.Format.PLAIN_TEXT || format == LogSource.Format.ZIP || format == LogSource.Format.GZIP) {
            return LogSource.lines(path);
        }
